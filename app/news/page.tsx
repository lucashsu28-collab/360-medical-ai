...
const CATEGORY_LABEL: Record<string, { label: string; color: string; bg: string }> = {
  domestic: { label: "國內醫美", color: "#C53030", bg: "#FFF5F5" },
  korea: { label: "韓國醫美", color: "#7C3AED", bg: "#F5F3FF" },
  international: { label: "國際新知", color: "#0E7490", bg: "#ECFEFF" },
  tech: { label: "新技術", color: "#D97706", bg: "#FFFBEB" },
};
...
function NewsCard({ news: n }: { news: NewsItem }) {
  ...
  return (
    ...
    >
      {/* 封面 */}
      <div style={{ aspectRatio: "16/10", background: cat.bg, position: "relative", overflow: "hidden" }}>
        {n.cover_image ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={n.cover_image} alt={n.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />
        ) : (
          <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 56, opacity: 0.35 }}>
            📰
          </div>
        )}
        {/* 分類徽章 */}
